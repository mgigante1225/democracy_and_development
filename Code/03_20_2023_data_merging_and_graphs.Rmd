...
df_penn_world <- penn_world%>% dplyr::select(country, countrycode, year, rgdpna)
...
df_merged %>% filter(country=="Chile")%>%
  ggplot(., aes(x = year)) + geom_line(aes(y=polity2), col="blue", size=2) + geom_line(aes(y=rgdpna/coef), col = "red", size=2) + scale_y_continuous("Polity Score", sec.axis = sec_axis(~., name="GDP")) + ggtitle("Graph comparing polity score and gdp value Chile")+ theme(axis.title.y = element_text(color = "blue", size=13), axis.title.y.right = element_text(color = "red", size=13))
```
